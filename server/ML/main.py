...
from ML.template import Template as Template
...
def process_dir(root_dir, subdir, template,ans):
    ...
    paths = config.Paths(os.path.join('output', subdir))   
    exts = ('*.png', '*.jpg')
    omr_files = sorted(
    ...
    if omr_files:
        ...
        # ans+= process_files(root_dir,omr_files, template, output_set),
        ans+= process_files(root_dir,omr_files, template),
    ...
    for folder in subfolders:
        process_dir(root_dir, os.path.join(subdir, folder), template,ans)
...
def processOMR(template, omrResp):
    ...
    # So its implementation is completely subjective to user's requirements.txt
    csvResp = {}
...
def process_files(root_dir,omr_files, template):
    start_time = int(time())
    filesCounter = 0
    filesNotMoved = 0
    ...
    for filepath in omr_files:
        ...
        OMRresponseDict, final_marked, MultiMarked, multiroll = \
            utils.readResponse(template, OMRCrop, name=file_id,
                         savedir=savedir, autoAlign=False)
...
if __name__ == "__main__":
    ...
    print("hackathons : ",args['input_dir'])
